...
function toggleTimeLimitModeUI() {
  ...
  // make the word list long enough so that no huma` typer can reach the end
...
input.addEventListener('keydown', (e) => {
  ...
  if (checkAnswerToIndex()) {
    ...
    if (e.keyCode == 8) {
      ...
      errors++;
      if (prompt.children[0].children[wordIndex].children[letterIndex - 1]) {
        prompt.children[0].children[wordIndex].children[letterIndex - 1].style.color = 'var(--error-color)';
      }
    }
